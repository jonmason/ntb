	...
	bool enable)
{
	...
	if (enable) {
		...
		snd_soc_update_bits(codec,
			RT5659_IRQ_CTRL_2, 0x8, 0x8);
		snd_soc_update_bits(codec,
			RT5659_4BTN_IL_CMD_2, 0x8000, 0x8000);
	} else {
...
static const struct snd_soc_dapm_widget rt5659_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA_S("Stereo1 ADC Volume L", 1,
		RT5659_STO1_ADC_DIG_VOL, RT5659_L_MUTE_SFT, 1,
		rt5659_adc_depop_event, SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_S("Stereo1 ADC Volume R", 1,
		RT5659_STO1_ADC_DIG_VOL, RT5659_R_MUTE_SFT, 1,
		rt5659_adc_depop_event, SND_SOC_DAPM_PRE_PMU),

	SND_SOC_DAPM_PGA_S("Mono ADC Volume L", 1,
		RT5659_MONO_ADC_DIG_VOL, RT5659_L_MUTE_SFT, 1,
		rt5659_adc_depop_event, SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_PGA_S("Mono ADC Volume R", 1,
		RT5659_MONO_ADC_DIG_VOL, RT5659_R_MUTE_SFT, 1,
		rt5659_adc_depop_event, SND_SOC_DAPM_PRE_PMU),
	...
	/* DAC channel Mux */
	SND_SOC_DAPM_MUX_E("DAC L1 Mux", SND_SOC_NOPM,
		0, 0, &rt5659_dac_l1_mux, rt5659_dac1_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_MUX("DAC R1 Mux", SND_SOC_NOPM, 0, 0, &rt5659_dac_r1_mux),
	SND_SOC_DAPM_MUX_E("DAC L2 Mux", SND_SOC_NOPM,
		0, 0, &rt5659_dac_l2_mux, rt5659_dac2_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MIXER("Mono DAC MIXR", SND_SOC_NOPM, 0, 0,
		rt5659_mono_dac_r_mix,
		ARRAY_SIZE(rt5659_mono_dac_r_mix)),
	SND_SOC_DAPM_MUX("DAC MIXL Mux", RT5659_DIG_MIXER,
		RT5659_M_DAC_MIX_L_SFT, 1, &rt5659_dig_dac_mixl_mux),
	SND_SOC_DAPM_MUX("DAC MIXR Mux", RT5659_DIG_MIXER,
		RT5659_M_DAC_MIX_R_SFT, 1, &rt5659_dig_dac_mixr_mux),
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (dapm->bias_level == SND_SOC_BIAS_OFF) {
			regmap_update_bits(rt5659->regmap,
				RT5659_PWR_DIG_1, RT5659_PWR_LDO,
				RT5659_PWR_LDO);
			regmap_update_bits(rt5659->regmap,
				RT5659_PWR_ANLG_1, RT5659_PWR_MB |
	...
	case SND_SOC_BIAS_OFF:
		regmap_update_bits(rt5659->regmap,
			RT5659_PWR_DIG_1, RT5659_PWR_LDO, 0);
...
void rt5659_micbias_output(int micbias, int enable_ldo2, int on)
{
	...
	if (on) {
		...
		if (enable_ldo2) {
			regmap_update_bits(global_regmap, RT5659_PWR_ANLG_3,
		...
		regmap_read(global_regmap, RT5659_PWR_ANLG_2, &value);
		if (!(value & (RT5659_PWR_MB1 | RT5659_PWR_MB2))) {
			regmap_update_bits(global_regmap, RT5659_PWR_ANLG_3,
				RT5659_PWR_LDO2, 0);
		}
		regmap_read(global_regmap, RT5659_PWR_DIG_1, &value);
		if (!(value & RT5659_PWR_LDO)) {
			regmap_update_bits(global_regmap,
				RT5659_PWR_ANLG_1, RT5659_PWR_MB |
				RT5659_PWR_VREF1 | RT5659_PWR_VREF2 |
				RT5659_PWR_FV1 |
				RT5659_PWR_FV2, 0);
