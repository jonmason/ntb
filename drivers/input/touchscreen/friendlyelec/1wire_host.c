 ...
 *
 * ChangeLog
 * 2017-08-28: Support for device tree
 *
...
/*
 * copy from drivers/pwm/pwm-samsung.c,
 * to control PWM directly for the lack of atomic pwm_enable()
 */
#define REG_TCON			0x08
#define REG_TINT			0x44
...
static inline void enable_tint(void)
{
	...
	tint = readl(pdata->pwm_reg_base + REG_TINT);
	tint |= (((1 << 5) | 1) << pdata->pwm_id);
	writel(tint, pdata->pwm_reg_base + REG_TINT);
}
...
#error "This driver requires PWM interrupt"

#endif /* !CONFIG_PWM_SAMSUNG */
...
static int onewire_map_dt_data(struct platform_device *pdev)
{
	...
	pdata->pwm_id = -1;
	pdata->pwm_irq = irq_of_parse_and_map(np, 0);
	if (pdata->pwm_irq <= 0) {
	...
	pdata->pwm_reg_base = devm_ioremap(&pdev->dev, res.start, resource_size(&res));
	if (IS_ERR(pdata->pwm_reg_base)) {
		dev_err(&pdev->dev, "Failed to ioremap memory\n");
		return PTR_ERR(pdata->pwm_reg_base);
	}

	pdata->gpio = of_get_named_gpio(np, "io,gpio", 0);
	if (!gpio_is_valid(pdata->gpio)) {
		dev_err(&pdev->dev, "cannot get io,gpio %d\n", pdata->gpio);
		return -ENODEV;
	}
...
#if defined(CONFIG_PWM_SAMSUNG)
	if (pdata->pwm_id < 0)
		pdata->pwm_id = pwm->hwpwm;
#endif

	ret = devm_gpio_request(&pdev->dev, pdata->gpio, TOUCH_DEVICE_NAME);
	if (ret) {
...
static struct of_device_id onewire_match_table[] = {
	{ .compatible = "onewire", },
	{ },
};
...
static struct platform_driver ts_1wire_device_driver = {
	...
	.driver		= {
		...
		.pm		= &ts_1wire_pm_ops,
#endif
		.of_match_table = of_match_ptr(onewire_match_table),
	}
};
...
static int onewire_ts_remove(struct i2c_client *client) {
