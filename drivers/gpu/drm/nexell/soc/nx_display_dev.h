 ...
 */
#ifndef __NX_DISPLAY_DEV_H__
#define __NX_DISPLAY_DEV_H__
...
struct disp_process_ops {
	int (*set_vsync)(struct disp_process_dev *dev,
					struct disp_vsync_info *vsync);
	int (*get_vsync)(struct disp_process_dev *dev,
					struct disp_vsync_info *vsync);
	int (*prepare)(struct disp_process_dev *dev);
	...
	void (*pre_resume)(struct disp_process_dev *dev);
	void (*resume)(struct disp_process_dev *dev);
	int (*capability)(struct disp_process_dev *dev,
					struct disp_vsync_info *vsync);
 ...
 */
/*
#define RET_ASSERT_VAL(_expr_, _ret_) do { \
	if (!(_expr_)) { \
		pr_err("%s(%d) : %s %s\n", \
 ...
 * number of RGB layer  : 0 = RGB0, 1 =RGB1
 * number of Video layer: 3 = VIDEO
 */
#define	LAYER_RGB_NUM			2
#define	LAYER_VID_NUM			1
...
#define	MLC_LAYER_RGB_0			0
#define	MLC_LAYER_RGB_1			1
...
struct disp_multily_dev {
	...
	int y_resol;
	int fb_layer;
	int video_prior;	/* 0=v>R0.., 1=R0>v>R1 .., 2=R0>R1>v */
	int enable;
	unsigned int bg_color;
	int interlace;
	int mem_lock_len;	/* lock size memory access, 4, 8, 16 */
	unsigned long base_addr;
	unsigned long save_addr;
	struct mlc_layer_info layer[MULTI_LAYER_NUM];
};
...
struct disp_lcd_param {
	...
	int invert_field;	/* 0= Normal, 1: Invert */
	int swap_RB;
	unsigned int yc_order;	/* for CCIR output */
...
struct disp_lvds_param {
	unsigned int lcd_format; /* 0:VESA, 1:JEIDA, 2: Location  */
	...
	int loc_enable;
	unsigned int loc_map[9]; /* lcd format is "Location Setting",*/
	unsigned int loc_mask[2]; /* lcd format is "Location Setting", 0 ~ 34 */
	unsigned int loc_pol[2]; /* lcd format is "Location Setting", 0 ~ 34 */
};
...
struct disp_resc_param {
	...
	int h_f_on;
	int v_f_on;
	unsigned int h_f_coe;	/* horizontal filter coefficient */
	unsigned int v_f_coe;	/* vertical   filter coefficient */
};
...
struct nx_lcd_plat_data {
	enum disp_dev_type display_in;	/* SYNCGEN0, SYNCGEN1 */
	enum disp_dev_type display_dev;	/* LCD, LVDS, MiPi, HDMI, ... */
	struct disp_vsync_info *vsync;
	struct disp_sync_par *sync_par;
	union disp_dev_param *dev_param;	/* LCD, LVDS, MiPi, HDMI,... */
};

#endif /* __NX_DISPLAY_DEV_H__ */
