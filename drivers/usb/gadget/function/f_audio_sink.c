 ...
 * Copyright (C) 2018 Hyunseok Jung <hsjung@nexell.co.kr>
...
static void audio_sink_free_inst(struct usb_function_instance *f)
{
	struct f_uac1_opts *opts;
...
#endif
static struct usb_function_instance *audio_sink_alloc_inst(void)
{
	struct f_uac1_opts *opts;
	struct device *dev;
	...
	opts->func_inst.free_func_inst = audio_sink_free_inst;
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
	dev = create_function_device("f_audio_sink");
...
static void audio_sink_free(struct usb_function *f)
{
	struct f_audio *audio = func_to_audio(f);
	struct f_uac1_opts *opts;
...
static struct usb_function *audio_sink_alloc(struct usb_function_instance *fi)
{
	...
	audio->card.func.free_func = audio_sink_free;
...
DECLARE_USB_FUNCTION_INIT(audio_sink, audio_sink_alloc_inst, audio_sink_alloc);
MODULE_LICENSE("GPL");
