 ...
 *
 * $Id: dhd_common.c 588552 2015-09-24 13:58:27Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#ifdef DHD_DEBUG
#include <sdiovar.h>
int dhd_print_d11core(dhd_pub_t *dhd_pub);
static obj_cache_t obj_cache = {0, 0, 0};
static int dhd_dump_d11core(dhd_pub_t *dhd_pub, uint8 type, uint16 addr, bool verbose, int idx);
static int dhd_read_ucode_d11obj32(dhd_pub_t *dhd_pub,
	uint16 objaddr, uint32 sel, bool verbose, int idx);
static int dhd_read_ucode_d11obj16(dhd_pub_t *dhd_pub,
	uint16 objaddr, uint32 sel, bool cache_valid, bool verbose, int idx);
#endif /* DHD_DEBUG */
...
static int
dhd_read_ucode_d11obj32(dhd_pub_t *dhd_pub, uint16 objaddr, uint32 sel, bool verbose, int idx)
{
	...
	if (verbose) {
		DHD_DUMP(("Indirect Write:\033[1m %x \033[0m,size %d=0x%x\n",
			(sel == OBJADDR_PSMXSCR_SEL)?(((objaddr >> 1) + 0x400) + CC_BASE):sel,
	...
	/* objaddr_read */
	if (verbose) {
		DHD_DUMP(("Indirect Read:\033[1m %x \033[0m,size %d=0x%x\n",
			(sel == OBJADDR_PSMXSCR_SEL)?(((objaddr >> 1) + 0x400) + CC_BASE):sel,
			sdreg.func, sdreg.value));
	}
	...
	switch (sel)
	{
		case OBJADDR_SCR_SEL: {
			DHD_DUMP((" %d scr \033[1m0x%-4x\033[0m = 0x%-8x\n", idx, objaddr, val));
			break;
		} case OBJADDR_SHM_SEL: {
			DHD_DUMP((" %d shm \033[1m0x%-4x\033[0m = 0x%-8x\n", idx, objaddr, val));
			break;
		} case OBJADDR_PSMXSCR_SEL: {
			DHD_DUMP((" %d scrx \033[1m0x%-4x\033[0m = 0x%-8x\n", idx, objaddr, val));
			break;
		} case OBJADDR_PSMXSHM_SEL: {
			DHD_DUMP((" %d shmx \033[1m0x%-4x\033[0m = 0x%-8x\n", idx, objaddr, val));
			break;
		} case OBJADDR_PSMXIHR_SEL: {
			DHD_DUMP(("%d ihrx16 \033[1m0x%-4x\033[0m = 0x%-8x\n", idx,
				((objaddr >> 1) + 0x400), val));
			break;
		}
		default: {
			DHD_ERROR(("%s: Unrecognized type \n", __FUNCTION__));
			break;
		}
	}
	return bcmerror;
...
static int
dhd_read_ucode_d11obj16(dhd_pub_t *dhd_pub, uint16 objaddr,
		uint32 sel, bool cache_valid, bool verbose, int idx)
{
	uint32 val;

	if (cache_valid && ((obj_cache.sel ^ sel) & (0xffffff)) == 0) {
		if (obj_cache.addr32 == (objaddr & ~0x3)) {
			if (verbose) {
				DHD_DUMP(("DEBUG: %s: Read cache value: addr32 0x%04x, "
					"sel 0x%08x, value 0x%08x\n",
					(sel & OBJADDR_SCR_SEL) ? "SCR":"SHM",
					obj_cache.addr32, obj_cache.sel, obj_cache.val));
			}
			val = dhd_read_ucode_d11obj32(dhd_pub, objaddr, sel, verbose, idx);
			goto exit;
		} else if ((obj_cache.sel & OBJADDR_RINC) &&
			(obj_cache.addr32 + 4 == (objaddr & ~0x3))) {
			if (verbose) {
				DHD_DUMP(("DEBUG: %s: Read objdata only: addr32 0x%04x, "
					"sel 0x%08x, value 0x%08x\n",
					(sel & OBJADDR_SCR_SEL) ? "SCR":"SHM",
					obj_cache.addr32, obj_cache.sel, obj_cache.val));
			}
			val = dhd_read_ucode_d11obj32(dhd_pub, 0xffff, sel, verbose, idx);
			goto exit;
		}
	}
	val = dhd_read_ucode_d11obj32(dhd_pub, objaddr, sel, verbose, idx);
exit:
	...
	obj_cache.sel = sel;
	obj_cache.val = val;
	return ((objaddr & 0x2) ? (val >> 16) : val);
}

static int
dhd_dump_d11core(dhd_pub_t *dhd_pub, uint8 type, uint16 addr, bool verbose, int idx)
{
	const char *name = "sbreg";
	uint32 val = 0;
	sdreg_t sdreg;
	...
	DHD_TRACE(("%s:  d11core Dump \n", __FUNCTION__));
		...
		switch (type) {
			case D11REG_TYPE_IHR32: {
				...
				bcmerror = dhd_bus_iovar_op(dhd_pub, name,
					&sdreg, sizeof(sdreg), (void *)&val,
					sizeof(sdreg_t), IOV_GET);
				if (bcmerror != BCME_OK) {
					DHD_ERROR(("%s: SBREG IOVAR error %d\n", __FUNCTION__,
							bcmerror));
					break;
				}
				DHD_DUMP((" %-3d ihr32 \033[1m0x%-4x\033[0m = 0x%-8x\n",
					idx, addr, val));
				break;
			}
			case D11REG_TYPE_IHR16: {
				...
				DHD_DUMP((" %-3d ihr16 \033[1m0x%-4x\033[0m = 0x%-8x\n",
					idx, addr, val));
					break;
			}
			case D11REG_TYPE_SCR: {
				if ((bcmerror = dhd_read_ucode_d11obj16(dhd_pub, (addr << 2),
					OBJADDR_SCR_SEL, (idx != 0), FALSE, idx)) != BCME_OK) {
					DHD_ERROR(("%s: D11REG_TYPE_SCR error %d\n",
						__FUNCTION__, bcmerror));
				}
				break;
			}
			case D11REG_TYPE_SHM: {
				if ((bcmerror = dhd_read_ucode_d11obj16(dhd_pub,
					addr, OBJADDR_SHM_SEL, (idx != 0), FALSE, idx))
					!= BCME_OK) {
					DHD_ERROR(("%s: D11REG_TYPE_SHM error %d\n",
						__FUNCTION__, bcmerror));
				}
				break;
			}
			case D11REG_TYPE_IHRX16: {
				if ((bcmerror = dhd_read_ucode_d11obj16(dhd_pub,
					(addr - 0x400) << 1,
						OBJADDR_PSMXIHR_SEL, (idx != 0), FALSE, idx))
					!= BCME_OK) {
					DHD_ERROR(("%s: D11REG_TYPE_IHRX16 error %d\n",
						__FUNCTION__, bcmerror));
				}
				break;
			}
			case D11REG_TYPE_SCRX: {
				if ((bcmerror = dhd_read_ucode_d11obj16(dhd_pub,
					addr << 2, OBJADDR_PSMXSCR_SEL, (idx != 0), FALSE, idx))
					!= BCME_OK) {
					DHD_ERROR(("%s: D11REG_TYPE_SCRX error %d\n",
						__FUNCTION__, bcmerror));
				}
				break;
			}
			case D11REG_TYPE_SHMX: {
				if ((bcmerror = dhd_read_ucode_d11obj16(dhd_pub,
					addr, OBJADDR_PSMXSHM_SEL, (idx != 0), FALSE, idx))
					!= BCME_OK) {
					DHD_ERROR(("%s: D11REG_TYPE_SHMX error %d\n",
						__FUNCTION__, bcmerror));
				}
				break;
			}
			default:
				DHD_DUMP(("Unrecognized type %d!\n",
...
int
dhd_print_d11core(dhd_pub_t *dhd_pub)
{
	int i;
	int idx = 0;
	uint32 length;

	length = dhd_pub->datalength/sizeof(d11regs_list_t);

	for (i = 0; i < length; i++) {
		switch (dhd_pub->ucode_psm_list[i].type) {
			case D11REG_TYPE_IHR32: {
				DHD_DUMP(("<< ihr32 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
					break;
				} case D11REG_TYPE_SCR: {
				DHD_DUMP(("<< scr 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
				...
				DHD_DUMP(("<< ihr16 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
				...
				DHD_DUMP(("<< shm 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
				...
				DHD_DUMP(("<< scrx 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
				...
				DHD_DUMP(("<< ihrx16 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
				...
				DHD_DUMP(("<< shmx 0x%-4x-b 0x%-4x -s %d -n %d >> \n",
					dhd_pub->ucode_psm_list[i].addr,
					dhd_pub->ucode_psm_list[i].bitmap,
					dhd_pub->ucode_psm_list[i].step,
					dhd_pub->ucode_psm_list[i].cnt));
		...
		if (dhd_pub->ucode_psm_list[i].bitmap) {
			while (dhd_pub->ucode_psm_list[i].bitmap) {
				if (dhd_pub->ucode_psm_list[i].bitmap & 0x1) {
					dhd_dump_d11core(dhd_pub,
						dhd_pub->ucode_psm_list[i].type,
						dhd_pub->ucode_psm_list[i].addr,
						FALSE, idx);
						idx++;
				}
				dhd_pub->ucode_psm_list[i].bitmap =
					dhd_pub->ucode_psm_list[i].bitmap >> 1;
				dhd_pub->ucode_psm_list[i].addr +=
					dhd_pub->ucode_psm_list[i].step;
			}
		}
		else {
			for (idx = 0; idx < dhd_pub->ucode_psm_list[i].cnt; idx++) {
				dhd_dump_d11core(dhd_pub, dhd_pub->ucode_psm_list[i].type,
						dhd_pub->ucode_psm_list[i].addr, FALSE, idx);
				dhd_pub->ucode_psm_list[i].addr +=
					dhd_pub->ucode_psm_list[i].step;
			}
		}
	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_ULIST: {
		/*
		 * Check if we already received the ucode_psm_list
		 * Also check if we received zero datalen.
		 */
		if (!(dhd_pub->ucode_psm_list) && datalen != 0) {
	...
	case WLC_E_UDUMP: {
		DHD_ERROR(("%s: Ucode Dump : Fatal reason = %d\n",
			__FUNCTION__, reason));
		if (dhd_print_d11core(dhd_pub) != BCME_OK) {
				return BCME_ERROR;
		}
		break;
	}
