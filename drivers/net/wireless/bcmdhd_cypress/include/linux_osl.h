...
#define PKTTAIL_WAR(osh, skb)		({BCM_REFERENCE(osh); (((struct sk_buff*)(skb))->tail);})
#define PKTEND_WAR(osh, skb)		({BCM_REFERENCE(osh); (((struct sk_buff*)(skb))->end);})
#define PKTHEAD_WAR(osh, skb)		({BCM_REFERENCE(osh); (((struct sk_buff*)(skb))->head);})
#define PKTDATA_WAR(osh, skb)		({BCM_REFERENCE(osh); (((struct sk_buff*)(skb))->data);})
#define PKTTRUESIZE_WAR(osh, skb)	\
	({BCM_REFERENCE(osh); \
	 (((struct sk_buff*)(skb))->truesize); \
	 })
#define PKTLEN_WAR(osh, skb)		({BCM_REFERENCE(osh); (((struct sk_buff*)(skb))->len);})
#define PKTDATALEN_WAR(osh, skb)	\
	({BCM_REFERENCE(osh); \
	 (((struct sk_buff*)(skb))->data_len); \
	 })

#define PKTSETTAIL_WAR(osh, skb, x)         \
	({ \
	 BCM_REFERENCE(osh); \
	 (((struct sk_buff*)(skb))->tail = (sk_buff_data_t)(x)); \
	 })

#define PKTSETDATA_WAR(osh, skb, x)         \
	({ \
	 BCM_REFERENCE(osh); \
	 (((struct sk_buff*)(skb))->data = (uint8 *)(x)); \
	 })
