 ...
 *
 * $Id: dhd_sdio.c 670540 2017-09-21 18:50:11Z $
 */
	...
	int *pad_pkt_len, void **new_pkt)
{
	...
	*new_pkt = NULL;
	...
	if (alloc_new_pkt) {
		...
		*new_pkt = tmp_pkt;
#if defined(CUSTOMER_AUTO_PLAT1) || defined(CUSTOMER_IMX)
		/* pull back sdpcm_hdrlen length from old skb as new skb here is passed
		 * to postprocessing
		 */
		PKTPULL(osh, pkt, sdpcm_hdrlen);
#endif /* CUSTOMER_AUTO_PLAT1 || CUSTOMER_IMX */
		pkt = tmp_pkt;
	}
...
#if defined(DISABLE_FLOW_CONTROL) || !defined(PLATFORM_MPS)
	dhd_doflow = FALSE;
#else
	dhd_doflow = TRUE;
