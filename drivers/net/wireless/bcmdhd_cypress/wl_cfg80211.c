 ...
 *
 * $Id: wl_cfg80211.c 674191 2017-10-11 22:35:19Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
u32 wl_dbg_level = WL_DBG_ERR | WL_DBG_DBG;  // htdo
...
)
{
...
#endif /* ((LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0)) && !defined(WL_COMPAT_WIRELESS)) */
	...
	goto set_channel;
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)) && !defined(WL_COMPAT_WIRELESS) */
...
static u32 wl_get_suite_auth_key_mgmt_type_wpa2(uint8 type)
{
	u32 ret = 0;
	switch (type) {
		case RSN_AKM_NONE:
	...
	bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	suite_count = ltoh16_ua(&mgmt->count);
	ptmp = (u8 *)&mgmt->list[0];
	wpa_auth2 = wl_get_suite_auth_key_mgmt_type_wpa2(ptmp[DOT11_OUI_LEN]);
	WL_ERR(("wpa ie wpa_suite_auth_key_mgmt count=%d, key_mgmt = 0x%X\n", count, wpa_auth2));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (n_pfn_results > 0) {
		...
		for (i = 0; i < n_pfn_results; i++) {
			...
			band = (channel_req <= CH_MAX_2G_CHANNEL) ? IEEE80211_BAND_2GHZ
				: IEEE80211_BAND_5GHZ;
